...
class BlenderMCPServer:
    ...
    def _log(self, cmd, status="ok", note=""):
        BlenderMCPServer._session_log.append({"cmd": cmd, "status": status, "note": note})
        if len(BlenderMCPServer._session_log) > 50:          # cap at 50 entries
            BlenderMCPServer._session_log = BlenderMCPServer._session_log[-50:]

    def get_session_log(self):
        """Return the last N commands executed this session (lightweight)."""
    ...
    def execute_code_safe(self, code, required_mode=None, push_undo=True):
        ...
        Returns same shape as execute_code plus mode info.
        """
        try:
            original_mode = None
            ...
            # --- restore mode ---
            if original_mode and obj and obj.mode != original_mode:
                try:
    ...
    def get_selection_context(self):
        ...
        try:
            ...
            # Edit-mode element counts (not the elements themselves — stays small)
    ...
    def get_scene_hierarchy(self):
        ...
        Enough for Claude to understand scene structure without flooding tokens.
        """
        try:
            def collection_info(col):
                return {
                    "name": col.name,
                    "objects": [{"name": o.name, "type": o.type} for o in col.objects],
                    "children": [collection_info(c) for c in col.children],
                }
    ...
    def get_mesh_quality_report(self, name):
        ...
        Call this explicitly when you need to diagnose a mesh.
        """
        try:
            import bmesh
            ...
            # isolated verts (no edge connections)
            isolated_verts = sum(1 for v in bm.verts if not v.link_edges)

            # poles (verts with edge count != 4, excluding boundary)
            poles_n3 = sum(1 for v in bm.verts if len(v.link_edges) == 3)
            poles_n5 = sum(1 for v in bm.verts if len(v.link_edges) == 5)
            poles_high = sum(1 for v in bm.verts if len(v.link_edges) > 5)
            ...
            # Overlapping UV check (basic: more than one UV layer with same verts mapped)
            ...
            report = {
                ...
                "poles": {"n3": poles_n3, "n5": poles_n5, "high": poles_high},
                "uv_layers": uv_layers,
                "has_uvs": has_uvs,
                "modifiers": modifiers,
                "health": "clean" if (non_manifold_edges == 0 and isolated_verts == 0
                                      and zero_area == 0 and ngons == 0) else "issues_found",
            }
    ...
    def get_material_graph(self, material_name):
        ...
        Call on-demand when you need to understand or edit a material.
        """
        try:
            ...
            for node in mat.node_tree.nodes:
                ...
                # grab key input values (skip linked ones — they come from connections)
                input_vals = {}
            ...
            return {"material": material_name, "nodes": nodes_out, "links": links_out}
        except Exception as e:
    ...
    def get_animation_data(self, name):
        ...
        try:
            ...
            result = {"object": name, "type": obj.type}
            ...
            if obj.animation_data and obj.animation_data.action:
                ...
                for fc in action.fcurves:
                    fcurves_summary.append({
                        ...
                        "range": [fc.keyframe_points[0].co[0], fc.keyframe_points[-1].co[0]]
                                   if fc.keyframe_points else [],
                    })
                result["action"] = {
                    "name": action.name,
                    "frame_range": list(action.frame_range),
                    "fcurve_count": len(action.fcurves),
                    "channels": fcurves_summary[:30],  # cap at 30 channels
                }

            # NLA tracks summary
            if obj.animation_data and obj.animation_data.nla_tracks:
                result["nla_tracks"] = [
                    {"name": t.name, "strips": len(t.strips)}
    ...
    # TOPOLOGY LAYER
    # ─────────────────────────────────────────────────────────────────────────
    def analyze_topology(self, name):
        """
        Professional topology analysis: edge flow, deformation readiness, problem areas.
        Returns a structured assessment — not raw mesh data.
        """
        try:
            import bmesh
            obj = bpy.data.objects.get(name)
            ...
            tris  = sum(1 for f in bm.faces if len(f.verts) == 3)
            ngons = sum(1 for f in bm.faces if len(f.verts) > 4)
            quad_ratio = round(quads / total_faces * 100, 1) if total_faces else 0

            # pole analysis
            pole_map = {}
            for v in bm.verts:
                valence = len(v.link_edges)
                pole_map[valence] = pole_map.get(valence, 0) + 1

            # edge loop density (avg edges per vert)
            avg_valence = round(sum(len(v.link_edges) for v in bm.verts) / total_verts, 2) if total_verts else 0

            # boundary edges (open mesh detection)
            boundary_edges = sum(1 for e in bm.edges if e.is_boundary)

            # shading issues: faces with bad normals relative to neighbours
            bad_normals = 0
            for f in bm.faces:
                for nbr_face in {linked_f for e in f.edges for linked_f in e.link_faces if linked_f != f}:
                    if f.normal.dot(nbr_face.normal) < -0.5:
                        bad_normals += 1
                        break
            ...
            score = 100
            if quad_ratio < 80:
                issues.append(f"Low quad ratio ({quad_ratio}%) — poor deformation topology")
                score -= 20
            if ngons > 0:
                issues.append(f"{ngons} ngons detected — will cause shading artifacts")
                score -= min(ngons * 2, 20)
            if pole_map.get(3, 0) > total_verts * 0.1:
                issues.append("High tri-pole count — may cause creasing on subdivision")
                score -= 10
            if pole_map.get(6, 0) + pole_map.get(7, 0) + pole_map.get(8, 0) > 5:
                issues.append("High-valence poles (6+) detected — complex topology")
                score -= 10
            if boundary_edges > 0:
                issues.append(f"{boundary_edges} boundary edges — open mesh (not watertight)")
                score -= 10
            if bad_normals > 0:
                issues.append(f"{bad_normals} face normal conflicts — shading errors likely")
                score -= 10

            rating = "excellent" if score >= 90 else "good" if score >= 70 else "acceptable" if score >= 50 else "poor"
    ...
    def _topology_recommendations(self, issues):
        ...
        recs = []
        for issue in issues:
            if "ngon" in issue.lower():
                recs.append("Select ngons (Select > Select All by Trait > Faces by Sides >4) and subdivide or knife-cut into quads")
            if "quad ratio" in issue.lower():
                recs.append("Run Retopology or use the Quad Remesher for cleaner quad flow")
            if "pole" in issue.lower() and "high-valence" in issue.lower():
                recs.append("Dissolve unnecessary edges to reduce pole valence below 5")
            if "boundary" in issue.lower():
                recs.append("Use Mesh > Clean Up > Fill Holes or manually close open edges")
            if "normal" in issue.lower():
                recs.append("Recalculate normals: Edit Mode > Mesh > Normals > Recalculate Outside")
        return recs

    def detect_mesh_problems(self, name):
        """
        Fast problem scanner. Returns only the problems found — empty list = clean.
        Designed to be called quickly before any operation to validate mesh state.
        """
        try:
            import bmesh
            obj = bpy.data.objects.get(name)
            ...
            return {"object": name, "clean": len(problems) == 0, "problems": problems}
        except Exception as e:
    ...
    def get_armature_info(self, name):
        ...
        Does NOT return pose matrices (too large) — returns names and relationships.
        """
        try:
            obj = bpy.data.objects.get(name)
                ...
                for pb in obj.pose.bones:
                    for c in pb.constraints:
                        constraints_out.append({
                            ...
                            "constraint": c.type,
                            "name": c.name,
                            "enabled": c.enabled if hasattr(c, 'enabled') else True,
                        })
    ...
    def analyze_animation_quality(self, name, frame_start=None, frame_end=None):
        """
        Animation quality check: detect foot sliding, broken IK, stiff channels,
        missing follow-through. Returns a concise findings list — not FCurve data.
        """
        try:
            obj = bpy.data.objects.get(name)
            if not obj:
            ...
            f_start = frame_start or int(scene.frame_start)
            f_end   = frame_end   or int(scene.frame_end)
            findings = []

            if not obj.animation_data or not obj.animation_data.action:
                return {"object": name, "findings": ["No animation data found"], "score": 0}

            action = obj.animation_data.action

            # --- Stiff channel detection (channels with very few keyframes) ---
            stiff = [fc.data_path for fc in action.fcurves
                     if len(fc.keyframe_points) < 3 and "rotation" in fc.data_path]
            if stiff:
                findings.append(f"Potentially stiff rotation channels ({len(stiff)} channels with <3 keys) — add breakdown poses")

            # --- Foot sliding (location channels with near-constant Z but changing XY) ---
            loc_z_curves = [fc for fc in action.fcurves
                            if 'location' in fc.data_path and fc.array_index == 2]
            for fc in loc_z_curves:
                vals = [fc.evaluate(f) for f in range(f_start, min(f_end, f_start + 50), 2)]
                if vals:
                    z_range = max(vals) - min(vals)
                    if z_range < 0.01:
                        findings.append(f"Possible foot sliding: '{fc.data_path}' Z barely moves — check foot lock")

            # --- Missing secondary motion (no scale animation on children) ---
            if obj.type == 'ARMATURE' and obj.pose:
                has_scale_anim = any('scale' in fc.data_path for fc in action.fcurves)
                if not has_scale_anim:
                    findings.append("No scale animation — consider adding squash/stretch for secondary motion")

            # --- Broken IK (pose bones with IK but no IK target set) ---
            if obj.type == 'ARMATURE' and obj.pose:
                for pb in obj.pose.bones:
                    for c in pb.constraints:
                        if c.type == 'IK' and not c.target:
                            findings.append(f"Broken IK on bone '{pb.name}' — IK constraint has no target")

            score = max(0, 100 - len(findings) * 15)
            rating = "excellent" if score >= 90 else "good" if score >= 70 else "needs_work"
            ...
            return {
                ...
                "score": score,
                "rating": rating,
                "recommendation": "Address findings above before export" if findings else "Animation looks healthy",
            }
        except Exception as e:
    ...
    def get_material_summary(self, name):
        ...
        try:
            ...
            return {"object": name, "material_count": len(slots), "materials": slots}
        except Exception as e:
    ...
    def create_pbr_material(self, name,
        ...
        wear_variation=True adds a noise-driven roughness variation for realism.
        Returns material name only — no node graph dump (stays token-light).
        """
        try:
            ...
            if wear_variation:
                # Noise → roughness variation — simulates surface wear without textures
                ...
                mix_rough.inputs[1].default_value = roughness - 0.15

                clamp = nodes.new('ShaderNodeClamp')
                clamp.location = (150, -100)
            ...
            return {"created": name, "wear_variation": wear_variation, "nodes_added": len(nodes)}
        except Exception as e:
    ...
    def run_asset_qa(self, name, check_uvs=True, check_materials=True, check_modifiers=True):
        ...
        Returns a pass/fail verdict with a concise issue list.
        Each check is individually controllable to keep responses lean.
        """
        try:
            obj = bpy.data.objects.get(name)
            if not obj:
            ...
            rot_clean  = all(abs(v) < 0.001 for v in obj.rotation_euler)
            scale_clean = all(abs(v - 1.0) < 0.001 for v in obj.scale)
            if not loc_clean:  issues.append("Location not at origin — apply or intentional?")
            else: passed.append("Location at origin")
            if not scale_clean: issues.append("Scale not applied (not 1,1,1) — apply scale before export")
            else: passed.append("Scale applied")
            ...
            if obj.type == 'MESH':
                ...
                nm = sum(1 for e in bm.edges if not e.is_manifold)
                if nm: issues.append(f"{nm} non-manifold edges — will cause export problems")
                ...
                iso = sum(1 for v in bm.verts if not v.link_edges)
                if iso: issues.append(f"{iso} isolated vertices — clean up mesh")
                else: passed.append("No isolated verts")

                face_count = len(bm.faces)
                if face_count > 50000:
                    issues.append(f"High poly count ({face_count} faces) — check LOD requirements")
                else:
                ...
                if check_uvs:
                    if not obj.data.uv_layers:
                        issues.append("No UV maps — required for texturing and Unreal export")
                    else:
                ...
                if check_materials:
                    if not obj.material_slots:
                        ...
                        empty_slots = [i for i, s in enumerate(obj.material_slots) if not s.material]
                        if empty_slots:
                            issues.append(f"Empty material slots at indices: {empty_slots}")
                        else:
                            passed.append(f"Materials OK ({len(obj.material_slots)} slot(s))")

                # --- Modifier check ---
                if check_modifiers:
                    unapplied = [m.name for m in obj.modifiers if m.show_viewport]
                    if unapplied:
                        issues.append(f"Unapplied modifiers: {unapplied} — apply before export")
                    else:
                        passed.append("No unapplied modifiers")
            ...
            return {
                ...
                "summary": f"{verdict} — {len(issues)} issue(s) found" if issues else f"{verdict} — asset is production ready",
            }
        except Exception as e:
    ...
    def run_unreal_readiness_check(self, name, expected_unit_scale=0.01):
        """
        Unreal Engine 5 specific pre-export checklist.
        Checks naming conventions, scale, triangulation, collision mesh presence,
        and LOD naming. Returns compact pass/fail per check.
        """
        try:
            obj = bpy.data.objects.get(name)
            if not obj:
            ...
            # UE5 naming convention check (SM_ SK_ T_ etc.)
            ue_prefixes = ("SM_", "SK_", "T_", "M_", "MI_", "BP_", "A_", "P_", "NS_")
            checks["naming_convention"] = {
                "pass": any(name.startswith(p) for p in ue_prefixes),
                "detail": f"Name '{name}' — UE5 expects prefixes like SM_ (static mesh) or SK_ (skeletal)",
            }

            # Scale check — Blender 1m = UE5 100cm, export scale should be 100
            scale = obj.scale
            scale_uniform = abs(scale.x - scale.y) < 0.001 and abs(scale.y - scale.z) < 0.001
            checks["scale_uniform"] = {
                "pass": scale_uniform,
                "detail": f"Scale {list(round(v,3) for v in scale)} — must be uniform for UE5",
            }

            # Pivot at origin (UE5 uses object pivot as actor pivot)
            pivot_ok = all(abs(v) < 0.01 for v in obj.location)
            checks["pivot_at_origin"] = {
                "pass": pivot_ok,
                "detail": "Pivot should be at world origin for predictable UE5 placement",
            }
            ...
            if obj.type == 'MESH':
                ...
                checks["triangulated"] = {
                    "pass": quads_ngons == 0,
                    "detail": f"{quads_ngons} non-tri faces — UE5 FBX import triangulates at runtime (pre-triangulate for control)",
                }

                # UV lightmap check (second UV channel for Lumen/baked lighting)
                uv_count = len(obj.data.uv_layers)
                checks["lightmap_uv"] = {
                    "pass": uv_count >= 2,
                    "detail": f"{uv_count} UV channel(s) — UE5 Lumen/lightmass needs a dedicated UV channel 1 for lightmaps",
                }
                bm.free()

                # Collision mesh check (UCX_, UBX_, USP_ prefix objects)
                col_name_variants = [f"UCX_{name}", f"UBX_{name}", f"USP_{name}",
                                     f"UCX_{name}_00", f"UBX_{name}_00"]
                has_collision = any(bpy.data.objects.get(cn) for cn in col_name_variants)
                checks["collision_mesh"] = {
                    "pass": has_collision,
                    "detail": "No collision mesh found — create UCX_[name] for custom collision in UE5",
                }

                # LOD naming check (_LOD0, _LOD1 etc.)
                lod0_name = f"{name}_LOD0"
                has_lod = bpy.data.objects.get(lod0_name) is not None
                checks["lod_naming"] = {
                    "pass": has_lod,
                    "detail": f"No '{lod0_name}' found — name LODs as [name]_LOD0, _LOD1 etc. for UE5 LOD import",
                }

                # No modifiers unapplied
                unapplied = [m.name for m in obj.modifiers]
                checks["modifiers_applied"] = {
                    "pass": len(unapplied) == 0,
                    "detail": f"Unapplied modifiers: {unapplied}" if unapplied else "All modifiers applied",
                }
            ...
            return {
                "object": name,
                "ue5_ready": passed_count == total,
                "passed": passed_count,
                "total_checks": total,
                "checks": checks,
                "summary": f"{passed_count}/{total} UE5 checks passed",
            }
        except Exception as e:
    ...
    def export_for_unreal(self, name, export_path,
        ...
        """
        Export a single object (or armature + meshes) as FBX with correct UE5 settings.
        scale=100 converts Blender metres → UE5 centimetres.
        Returns export path and stats — no mesh data.
        """
        try:
            obj = bpy.data.objects.get(name)
            if not obj:
            ...
            # Select only this object (and its children for skeletal meshes)
            ...
            bpy.ops.export_scene.fbx(
                ...
                axis_forward='-Z',
                axis_up='Y',
                object_types={'MESH', 'ARMATURE'} if export_animations else {'MESH'},
    ...
    def prepare_lod_names(self, base_name, lod_count=4):
        """
        Scan scene for LOD objects following UE5 convention and report their status.
        Also creates correctly named empty placeholders if LODs are missing.
        Returns a compact status table — never mesh data.
        """
        try:
            lods = []
            for i in range(lod_count):
                lod_name = f"{base_name}_LOD{i}"
                exists = bpy.data.objects.get(lod_name) is not None
                face_count = 0
                if exists:
                    lod_obj = bpy.data.objects[lod_name]
            ...
            return {
                ...
                "lods": lods,
                "missing": missing,
                "tip": "Rename your LOD meshes to match the expected names above for UE5 import",
            }
        except Exception as e:
